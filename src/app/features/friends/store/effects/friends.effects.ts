...
export class FriendsEffects {
    ...
    loadFriends$ = createEffect(() => this.actions$.pipe(
        ...
        mergeMap(([action, {currentPage, pageCount}]) => (currentPage === 0 || currentPage < pageCount)
            ? this.friendsApiService.getFriendListPage$(currentPage + 1, action.pageSize).pipe(
                ...
                catchError((({code, description}) => of(FriendsActions.loadFriendListNextPageError({
                    code,
                    description
                }))))
            )
